...
export const YMapHintContext = createContext<YMapHintContextType>({
  hint: undefined,
});
...
const YMapHintWrapper: React.FC<YMapHintWrapperProps> = ({
  children,
  context,
}) => {
  ...
  return (
    <YMapHintContext.Provider value={hint}>{children}</YMapHintContext.Provider>
  );
};
...
const YMapHint: React.FC<YMapHintProps> = ({ children, hint }) => {
  ...
  useEffect(() => {
    if ((window as any)[Package.Hint]) {
      ...
      ymaps.import(Package.Hint as any).then((data) => {
  ...
  return (
    <YMapHintComponent hint={hint}>
      <YMapHintWrapper context={context}>{children}</YMapHintWrapper>
    </YMapHintComponent>
  );
};
